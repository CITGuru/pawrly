...
type Lang = "sql" | "yaml" | "bash";

type Rule = { re: RegExp; cls: string };

// Lightweight, display-only tokenizers. Not a real parser — just enough to give
// the marketing code blocks a calm, legible tint. Rules are tried in order at
// each cursor position; first match at the cursor wins.
const RULES: Record<Lang, Rule[]> = {
  ...
  sql: [
    ...
    {
      re: /\b(SELECT|FROM|JOIN|LEFT|RIGHT|INNER|OUTER|ON|WHERE|GROUP|BY|ORDER|ASC|DESC|AND|OR|NOT|AS|COUNT|SUM|AVG|MIN|MAX|INTERVAL|LIMIT|HAVING|DISTINCT|true|false|null)\b/iy,
      cls: "tok-key",
    },
...
export function CodeBlock({
  ...
  return (
    <div className={`code-surface overflow-hidden rounded-2xl soft-shadow ${className}`}>
      ...
      <pre className="overflow-x-auto px-4 py-4 font-mono text-[13px] leading-[1.65] text-cream">
        <code>
          {lines.map((line, idx) => (
            <div key={idx} className="whitespace-pre">
              {line.length ? tokenizeLine(line, rules) : " "}
            </div>
          ))}
        </code>
      </pre>
