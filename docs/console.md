...
The **Console** is a browser UI for managing Pawrly: browse your sources and their health, the SQL catalog, the semantic layer, the cache and materialized tables, recent query activity, and run ad-hoc SQL with live-streaming results.

It is a static single-page app that calls the Pawrly engine over gRPC-Web.
...
`pawrly console` uses the standard [CLI](./cli.md) workspace resolution order: `--config`, `$PAWRLY_CONFIG`, `./pawrly.yaml`, then `~/.pawrly/pawrly.yaml`. A loopback listener does not require a token.
...
| **Semantic**     | [Semantic models](./semantic.md), dimensions, and measures. Click a model to inspect it or run a query by selecting measures and dimensions.               |
...
The Console UI uses gRPC-Web. The [REST API](./api.md) is available on the same host and port for scripts and other HTTP clients, but the browser UI does not use it.

The Console connects to **one daemon at a time**, set by the **Endpoint** field in the sidebar:

- **Embedded (default).** When the daemon serves the Console itself (`pawrly console`), the UI uses that same origin.
...
The Console uses the HTTP listener's bearer-token, bind-address, and CORS settings.
...
The official binaries bundle the Console. To build it yourself, build the Console app first, then compile it into the binary with the `console` feature:
